/*
 * SKUD Enhanced - Система контроля и учета доступа с поддержкой RC522 (Mifare 13.56 МГц)
 * Версия для работы с Python-сервером Enhanced
 * 
...
// Python API URL - обновленный URL для Enhanced версии
const char* serverURL = "http://194.87.43.42:5000/api/attendance";
// Альтернативные URL для тестирования:
// const char* serverURL = "http://localhost:5000/api/attendance";  // Локальная разработка
// const char* serverURL = "https://yourdomain.com/api/attendance"; // Продакшн
...
void setup() {
  Serial.begin(115200);
  Serial.println("СКУД Enhanced система запущена");
  Serial.println("Версия: Enhanced с улучшенной архитектурой");
  ...
  Serial.println("[RC522] ВНИМАНИЕ: Используются пины GPIO6,7,8,9,10 - возможны конфликты на ESP32-C3-MINI-1!");
  ...
  if (ENABLE_OTA) {
    ...
    ArduinoOTA.setHostname("SKUD-Enhanced-ESP32");
    ...
    ArduinoOTA.begin();
    Serial.println("[OTA] OTA инициализирован (hostname: SKUD-Enhanced-ESP32, port: 3232)");
  } else {
  ...
  Serial.println("=== СКУД Enhanced готова к работе ===");
}
...
void loop() {
  ...
  if (millis() - lastDebugTime > 5000) {
    ...
    Serial.println("[STATUS] Проверка состояния системы Enhanced...");
  ...
  if (cardDetected && serial.length() > 0) {
    ...
    if (LOCAL_MODE) {
      ...
      // Отправляем данные на Python-сервер Enhanced
      Serial.println("[API] Отправка данных на Enhanced сервер...");
      Serial.println("[API] URL: " + String(serverURL));
    ...
    if (response.length() > 0) {
      StaticJsonDocument<512> doc; // Увеличил размер для Enhanced ответов
      ...
      if (!error) {
        ...
        if (status == "success") {
          String employee = doc["employee"];  // Совместимость со старым API
          String event = doc["event"];        // Совместимость со старым API
          String time = doc["time"];          // Совместимость со старым API
...
/**
 * Отправка данных на Python Enhanced сервер
...
String sendToPythonServer(String serial, String time) {
  ...
  http.addHeader("User-Agent", "ESP32-SKUD-Enhanced/1.0");
  ...
  if (httpCode > 0) {
    ...
    if (httpCode == -1) errorMsg = "Connection failed";
    else if (httpCode == -11) errorMsg = "Timeout";
    
    response = "{\"status\":\"error\",\"message\":\"" + errorMsg + "\"}";
    Serial.println("HTTP ошибка: " + errorMsg);
  }
